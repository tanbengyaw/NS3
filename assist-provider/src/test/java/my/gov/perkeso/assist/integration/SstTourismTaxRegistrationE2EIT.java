...
@ActiveProfiles("test")
class SstTourismTaxRegistrationE2EIT {
    ...
    @Test
    void roSubmitAutoApprovesTourismTaxWithSmkAndPremises() {
        ...
        final JsonNode created = ro.createCase("""
                {
                  "sectionId": 1101,
                  "employerName": "Tourism Hotel Sdn Bhd",
                  "registrationNo": "%s",
                  "serviceTypeId": 1,
                  "pksBranchId": 2,
                  "postCode": "50812",
        ...
        ro.updateCase(caseId, """
                {
                  ...
                  "addressLine1": "1 Jalan Beach"
                }
                """);

        ro.upsertSstInfo(caseId, """
                {
                  "tradeName": "Tourism Hotel",
                  "form1ContactPerson": "Siti Aminah",
                  "finYrEndMon": 12,
        ...
        assertThat(approved.get("changes").get("tourismTaxSmkRegNo").asText()).contains("-CO-");
        assertThat(approved.get("changes").get("sstSmkRegNo").asText())
                .isEqualTo(approved.get("changes").get("tourismTaxSmkRegNo").asText());

        final var sstInfoRows = sstInfoRepository.findByRegGeneralInfoIdAndDeletedFalse(caseId);
        assertThat(sstInfoRows).hasSize(1);
        assertThat(sstInfoRows.get(0).getTourismTaxSmkRegNo()).contains("-CO-");
        assertThat(sstInfoRows.get(0).getSmkRegNo()).isEqualTo(sstInfoRows.get(0).getTourismTaxSmkRegNo());
        ...
        assertThat(statuses.get(0).getTaxTypeId()).isEqualTo(TaxType.TOURISM_TAX.getAssistId());

        final String tourismSmk = approved.get("changes").get("tourismTaxSmkRegNo").asText();
        ...
        assertThat(letterHtml).contains(tourismSmk);
    }
